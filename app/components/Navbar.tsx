          ...
          <div className="flex items-center space-x-3">
            ...
            {cvList.length > 0 && (
              <select
                ...
                className="bg-white/10 text-white text-sm rounded-lg px-3 py-1.5 border border-white/20 hover:bg-white/20 focus:outline-none focus:ring-2 focus:ring-white/30 cursor-pointer max-w-[200px]"
              >
                ...
                {cvList.map((cv, index) => {
                  const id = getCvId(cv);
                  const label = String(cv.Label ?? cv.label ?? "");
                  return (
                    <option
                      key={`${id || "cv"}-${index}`}
                      value={id}
