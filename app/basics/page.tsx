...
export default function BasicsPage() {
  ...
  const { basics, setBasics, sharedBackendData } = useCVStore();
  // const basics = resume.basics;
  // const setBasics = (newBasics: Basics) => setResume({ ...resume, basics: newBasics });
  const [isEditing, setIsEditing] = useState<boolean>(false);
  const [isHydratedFromBackend, setIsHydratedFromBackend] = useState(false);

  useEffect(() => {
    if (!sharedBackendData || isHydratedFromBackend) {
      return;
    }
console.log(
  "Hydratation des données du backend dans le store...",
  sharedBackendData,
);
    const source = sharedBackendData as Record<string, unknown>;
    const backendBasics =
      (source.basics as Record<string, unknown> | undefined) ?? source;
    const backendLocation =
      (backendBasics.location as Record<string, unknown> | undefined) ?? {};
    ...
    setBasics({
      ...
      url: String(backendBasics.url ?? basics.url ?? ""),
      summary: String(backendBasics.summary ?? basics.summary ?? ""),
      location: {
        address: String(
          backendLocation.address ?? basics.location?.address ?? "",
        ),
        postalCode: String(
          backendLocation.postalCode ?? basics.location?.postalCode ?? "",
        ),
        city: String(backendLocation.city ?? basics.location?.city ?? ""),
        countryCode: String(
          backendLocation.countryCode ?? basics.location?.countryCode ?? "",
        ),
        region: String(backendLocation.region ?? basics.location?.region ?? ""),
      },
    });

    setIsHydratedFromBackend(true);
  }, [sharedBackendData, isHydratedFromBackend, setBasics, basics]);
              ...
              <div className="flex items-center space-x-6">
                ...
                <div>
                  <h2 className="text-3xl font-bold">
                    {basics.name || "Nom non défini"}
                  </h2>
                  <p className="text-blue-100 text-lg mt-1">
                    {basics.label || "Titre professionnel"}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="p-8 space-y-6">
              ...
              {basics.summary && (
                <div className="pt-6 border-t border-gray-200">
                  ...
                  <p className="text-gray-700 leading-relaxed whitespace-pre-line pl-7">
                    {basics.summary}
                  </p>
                </div>
              )}
            ...
            <div className="bg-gray-50 rounded-lg p-6 border border-gray-200">
              ...
              <FormSection
                ...
                onSubmit={(data) => {
                  setBasics({
                    ...basics,
                    location: converDataToLocation(data),
                  });
                  setIsEditing(false);
                }}
                onCancel={() => setIsEditing(false)}
